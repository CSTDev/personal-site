...
5. Select the port by going to Tools → Port. The ESP8266 typically appears as `COM3` on Windows or `/dev/ttyUSB0` on Linux. If you're unsure which port is correct, unplug the board, check which ports are listed, then plug it back in — the new entry is your board.
